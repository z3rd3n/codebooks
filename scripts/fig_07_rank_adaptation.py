...
R16 eType II on a 4-rx, 6-ray channel:
...
Type I / R15 stop at rank 2 (spec); their fixed-rank curves are in fig_01.
...
def main() -> None:
    ...
    scheme = R16Type2Codebook(ANT, N3=N3, param_combination=6)

    se_fixed = {r: np.zeros(len(SNR_DB)) for r in RANKS}
    se_auto = np.zeros(len(SNR_DB))
    ...
    for _ in range(args.drops):
        ...
        W_fixed = {r: scheme.precoder(scheme.select(H, rank=r)) for r in RANKS}
        W_ub = eigen_precoder(H, rank=4)
        for i, snr in enumerate(SNR_DB):
            rho = 10 ** (snr / 10)
            for r in RANKS:
                se_fixed[r][i] += su_rate(H, W_fixed[r], rho)
            rank, _, W, se = select_rank(scheme, H, rho=rho, ranks=RANKS)
            se_auto[i] += se
            picks[i, rank - 1] += 1
    ...
    for r, shade in zip(RANKS, shades):
        axes[0].plot(SNR_DB, se_fixed[r] / args.drops, color=shade, marker="^",
                     label=f"R16, fixed rank {r}")
    axes[0].plot(SNR_DB, se_auto / args.drops, color="#D95319", marker="o",
                 linewidth=2, label="R16, auto-RI")
    ...
    axes[1].set_ylim(0, 1)
    axes[1].set_title("rank chosen by auto-RI")
    axes[1].legend(fontsize=8, loc="center left")
    axes[1].grid(alpha=0.3)

    fig.suptitle(f"Rank adaptation -- R16 eType II pc6, 4 rx, 6-ray channel, "
                 f"{args.drops} drops")
    save(fig, args.out, "fig_07_rank_adaptation", {
        "snr_db": SNR_DB,
        "se_fixed": {r: list(se_fixed[r] / args.drops) for r in RANKS},
        "se_auto": list(se_auto / args.drops),
