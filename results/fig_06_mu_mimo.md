...
gNB zero-forces across the reported directions (ε-regularized, ε = 10⁻⁶);
...
* **The K sweep separates the families further.** R16 keeps growing to
  ...
  reported directions make the ZF Gram matrix near-singular, and the
  ε-regularized solver then lets those users interfere fully — the
  physically right outcome (plain ZF simply crashes; that is why the
  ε is there). Type I cannot support dense MU-MIMO, which is precisely
  the paper's motivation for Type II ("higher data rates and system
  capacity, eMBB").
